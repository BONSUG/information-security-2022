...
def pass_wheels(input, reverse=False):
    ...
    if reverse == True:
        ch=ETW[SETTINGS["WHEELS"][2]["wire"].index(input)]
        ch = ETW[SETTINGS["WHEELS"][1]["wire"].index(ch)]
        input = ETW[SETTINGS["WHEELS"][0]["wire"].index(ch)]
    else:
        ch = SETTINGS["WHEELS"][0]["wire"][ord(input) - ord('A')]
        ch = SETTINGS["WHEELS"][1]["wire"][ord(ch) - ord('A')]
        input = SETTINGS["WHEELS"][2]["wire"][ord(ch) - ord('A')]
...
for ch in plaintext:
    ...
    encoded_ch = pass_ukw(encoded_ch)  # 다시
