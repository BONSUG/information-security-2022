...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    for i in range(0,len(text),8):
        result+=sdes(text[i:i+8],key,MODE_ENCRYPT)
    return result

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    result = bitarray()
    for i in range(0, len(ciphertext), 8):
        result += sdes(ciphertext[i:i + 8], key, MODE_DECRYPT)
    return result
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    if len(iv)<8:
        temp = []
        for i in range(0,8-len(iv)):
            temp.append(0)
        iv = bitarray(temp)+iv

    for i in range(0,len(text),8):
        if i==0:
            result+=(sdes((text[i:i+8]^iv),key,MODE_ENCRYPT))
        else:
            result+=(sdes((text[i:i+8]^result[i-8:i]),key,MODE_ENCRYPT))
    return result
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    for i in range(0, len(ciphertext), 8):
        if i == 0:
            result+=(sdes(ciphertext[i:i + 8], key, MODE_DECRYPT))^iv
        else:
            result+=(sdes(ciphertext[i:i + 8], key, MODE_DECRYPT))^ciphertext[i-8:i]
    return result
