...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    if mode == MODE_ENCRYPT:
        ...
        a,b= temp[:4],temp[4:]
        roundkey = schedule_keys(key)
        rounded_b=round(b,roundkey[0])
        a ^= rounded_b
        #ba
        temp2= bitarray()
        rounded_a = round(a,roundkey[1])
        b^=rounded_a
        temp2=b+a
        for idx in IP_1:
            result.append(temp2[idx])
        ...
        a, b = temp[:4], temp[4:]
        roundkey = schedule_keys(key)
        rounded_b = round(b, roundkey[1])
        a ^= rounded_b
        # ba
        temp2 = bitarray()
        rounded_a = round(a, roundkey[0])
        b ^= rounded_a
        temp2 = b + a
        for idx in IP_1:
            result.append(temp2[idx])
